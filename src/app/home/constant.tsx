...
export const SKILLS_DATA: string[] = [
  ...
  'Production AI Architecture', 'Context Window Management', 'Hybrid Search (BM25 + Vector)', 'Retrieval Reranking',
  'Model Routing & Fallbacks', 'Semantic Caching', 'Prompt Prefix Caching',
  'Output Guardrails', 'AI Observability & Tracing', 'Evaluation-as-Infrastructure',
  'LLM-as-Judge', 'Human-in-the-Loop (HITL)', 'Durable Agentic Workflows', 'Async AI Pipelines',
  // Vector & Search
...
export const PROJECT_DATA: Project[] = [
  {
      ...
      ongoing: false,
      category: 'ai',
      description: 'Production SaaS that converts a 10-minute business interview into a complete 90-day go-to-market strategy — deployed live on Google Cloud Run + Firebase Hosting. Architected a stateless, horizontally-scalable orchestration layer with a provider-abstract LLM interface, enabling model routing and graceful fallback across providers without touching agent logic. Engineered a 10-agent AI pipeline (adaptive interview → embedding-ranked competitive scan → TAM/SAM market analysis → brand positioning → psychographic personas → channel strategy → 90-day roadmap → DALL-E 3 content studio) with Pydantic-validated JSON outputs, schema-validated retries, and deterministic fallbacks ensuring zero downtime. Built pgvector RAG memory (1536-dim embeddings, cosine similarity retrieval) and SHA-256 exact-match response cache (24h TTL) — reducing redundant LLM API calls by ~60%. Applied 3-stage output guardrails on every LLM response before serving to end users. Shipped a full MLOps observability stack: 7 Prometheus metrics, Sentry APM, per-agent cost tracking, output quality scoring (0.0–1.0), and pipeline step tracing.',
      skills: ['FastAPI', 'OpenAI API', 'pgvector', 'PostgreSQL', 'React', 'Docker', 'Prometheus', 'Sentry', 'DALL-E 3', 'Multi-Agent Systems', 'RAG', 'Output Guardrails', 'Stateless Orchestration', 'Model Routing', 'Semantic Caching', 'LLM Observability', 'Cloud Run'],
  ...
  },
  {
      ...
      ongoing: true,
      category: 'ai',
      description: 'Developed a multiagent RAG chatbot system using LangGraph and Gemini LLM with ReACT agents to intelligently process documents from PDFs, websites, and databases. Implemented a multi-stage retrieval funnel: broad vector retrieval via ChromaDB, followed by context budget management that prioritizes top-scored chunks while reserving output space — preventing context window overflow on long documents. Built explicit prompt assembly layer ordering context by importance (system instructions → retrieved chunks → recent history → older history). Full-stack: React/Next.js frontend, FastAPI backend with secure auth, document upload/analysis interface, and persistent memory enabling personalized responses based on user background.',
      skills: ['LangGraph', 'Gemini API', 'ReACT Agents', 'ChromaDB', 'FastAPI', 'Next.js', 'React', 'RAG', 'Context Window Management', 'Multi-stage Retrieval', 'Vector Embeddings', 'Multi-Agent Systems', 'Python'],
      githubUrl: 'https://github.com/Hemkush',
  },
  {
  ...
  },
  {
      ...
      ongoing: false,
      category: 'ai',
      description: 'Designed and developed a RAG-powered AI chatbot for UMD Smith School faculty and administrative staff. Conducted stakeholder discovery sessions to define requirements: policy summarization with source attribution, admin document management panel, and guided chat interface. Implemented RAG architecture with context budget management — building an explicit prompt assembly layer that prioritizes retrieved policy chunks over conversation history and reserves output space, preventing context overflow on large document corpora. Applied Agile methodology with weekly stakeholder syncs. Stack: React + Tailwind (frontend), Flask (backend), OpenAI/Anthropic (LLM), sentence-transformers (embeddings).',
      skills: ['RAG', 'Context Budget Management', 'Flask', 'React', 'Tailwind CSS', 'OpenAI API', 'Anthropic API', 'sentence-transformers', 'System Design', 'Figma', 'Python'],
      githubUrl: 'https://github.com/Hemkush',
  },
  {
      ...
      ongoing: false,
      category: 'ai',
      description: 'Built a production-grade AI portfolio using Next.js 15 App Router, TypeScript, and Tailwind CSS. Upgraded the AI chatbot from flat vector RAG to a full Microsoft GraphRAG-style pipeline: Gemini LLM auto-extracts typed entities (PERSON, ORGANIZATION, PROJECT, TECHNOLOGY, SKILL, ROLE, CONCEPT) and relationships from portfolio content at reindex time, persisted in PostgreSQL graph tables (graph_nodes, graph_edges, graph_communities). Union-Find algorithm groups connected nodes into communities; communities are vector-embedded and summarized by Gemini. At query time, three parallel retrieval legs run concurrently — local entity graph traversal (1–2 hops), global community similarity via pgvector embedding, and standard vector cosine search — then merged through token-budget-capped context assembly (vector 1,800 / entity 800 / community 600 tokens) with cross-section deduplication. The system auto-selects one of four query-adaptive modes: graphrag_hybrid, graphrag_local, graphrag_global, or vector_only fallback. Built a live AI Landscape feed aggregating 6 RSS sources hourly with ISR edge caching, keeping Gemini API calls under 24/day while serving all visitors in under 30ms. Includes Vitest unit-test suite, graph-status API, Google OAuth (NextAuth v5), and full mobile-responsive navigation.',
      skills: ['Next.js 15', 'TypeScript', 'React', 'Tailwind CSS', 'Google Gemini API', 'GraphRAG', 'Knowledge Graphs', 'Community Detection', 'Union-Find', 'Multi-hop Graph Traversal', 'Hybrid Retrieval', 'Context Budget Management', 'RAG', 'pgvector', 'Graph Schema Design', 'ISR / Edge Caching', 'NextAuth', 'Vitest', 'Vercel', 'Full-Stack Development'],
...
export const WORK_EXPERIENCE_DATA: WorkExperience[] = [
    {
        ...
        description: [
            ...
            'Architected the orchestration layer as a stateless, horizontally-scalable service with a provider-abstract LLM interface — enabling model routing and graceful fallback across providers without touching agent logic, with schema-validated retries and idempotency keys on all side-effecting calls.',
            ...
            'Applied 3-stage output content guardrails on every LLM response before serving to end users — enforcing safety, business-scope constraints, and schema compliance, with per-trigger audit logging.',
            ...
            'Engineered a full MLOps observability stack: 7 Prometheus metrics (LLM cost, latency, quality scores, cache hit rate), Sentry APM, per-agent output quality scoring (0.0–1.0 schema completeness), and pipeline step tracing with duration_ms logging.',
            'Implemented pgvector RAG memory (1536-dim embeddings, top-k cosine retrieval) and a DB-backed SHA-256 exact-match response cache (24h TTL) — reducing redundant LLM API calls by ~60% in production.',
        ...
        skills: ['FastAPI', 'React', 'PostgreSQL', 'OpenAI API', 'LangChain', 'Docker', 'Alembic', 'Multi-Agent Systems', 'RAG', 'Output Guardrails', 'Stateless Orchestration', 'Model Routing', 'Semantic Caching', 'LLM Observability', 'Prompt Engineering', 'Vector Memory', 'CI/CD', 'Python'],
        caseStudyUrl: '/marketpilot-showcase.html',
        liveUrl: 'https://ai-marketing-prod.web.app/',
    },
    {
        ...
        description: [
            ...
            'Proposed and implemented a high-level system design based on Retrieval-Augmented Generation (RAG) architecture with context budget management — building an explicit prompt assembly layer that prioritizes retrieved policy chunks, reserves output space, and prevents context window overflow on large document corpora.',
        ...
        skills: ['RAG', 'Context Budget Management', 'Flask', 'React', 'Tailwind CSS', 'OpenAI API', 'Anthropic API', 'sentence-transformers', 'Figma', 'Agile', 'System Design', 'Stakeholder Management'],
    },
    {
        company: 'University of Maryland — Robert H. Smith School of Business',
